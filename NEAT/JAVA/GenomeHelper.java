...
public class GenomeHelper
{
  ...
  public static Genome Crossover(Genome g1, Genome g2, float g1fitness, float g2fitness, Random r)
  {
    if (g1 == null || g2 == null)
    {
    ...
    crossover = new Genome(nodes, g1.GetMaxMutationAttempts());
    ...
    for (int i = 0; i < matchingconns1.size(); i++)
    {
      crossover.AddConnection(new Connection(matchingconns1.get(i).GetSourceNodeID(), matchingconns1.get(i).GetTargetNodeID(), 
        r.nextFloat() > 0.5 ? matchingconns1.get(i).IsExpressed : matchingconns2.get(i).IsExpressed, matchingconns1.get(i).GetInnovationnumber(),
        r.nextFloat() > 0.5 ? matchingconns1.get(i).Weight : matchingconns2.get(i).Weight));
    }

    for (int i = 0; i < matchingbiases1.size(); i++)
    {
      crossover.AddBias(new Bias(matchingbiases1.get(i).GetNode(), r.nextFloat() > 0.5 ? matchingbiases1.get(i).Value
    ...
    {
      crossover.AddConnection(new Connection(c.GetSourceNodeID(), c.GetTargetNodeID(), c.IsExpressed, c.GetInnovationnumber(), c.Weight));
    }
    ...
    {
      crossover.AddBias(new Bias(b.GetNode(), b.Value, b.GetInnovationnumber()));
    }
