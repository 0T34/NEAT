...
class NetworkPrinter {
    ...
    void InsertNode(HashMap<Node, Integer> layermap, Node nodetoinsert, int layer) {
    ...
    void InsertNode2(HashMap<Node, Integer> layermap, Node nodetoinsert, int layer) {
        layermap.put(nodetoinsert, layer);
        
        ArrayList<Connection> connections = this.genome.FindConnectionsFromNode(nodetoinsert.id);
        for (Connection connection : connections) {
            Node nodenextlayer = this.genome.GetNode(connection.target_node_id);
            // Output Nodes are added last
            if (nodenextlayer.type == Nodetype.Hidden) {
                InsertNode(layermap, nodenextlayer, layer + 1);  
            }
        }
    }
    
    NetworkPrinter(Genome g) {
        this.SetGenome(g);
        this.Recalculate();
    }
